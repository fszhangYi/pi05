...
class Pi05Inference:
    ...
    def __init__(
        ...
        if trt_engine or trt_build_engine:
        ...
        if trt_engine or trt_build_engine:
            from pi05_jax_sft.trt_policy import attach_tensorrt_policy
            ...
            print("Preparing TensorRT policy...")
            self._policy = attach_tensorrt_policy(
                self._policy,
                engine_path=trt_engine,
                build_engine_path=trt_build_engine,
                example_obs=dummy_obs,
                device=device,
                num_steps=trt_num_steps,
                fp16=not trt_fp32,
            )
            if trt_engine:
                print(f"Loaded TensorRT engine: {trt_engine}")
            if trt_build_engine:
                print(f"Built TensorRT engine: {trt_build_engine}")
        ...
        self._infer_chunk_raw({"top_image": dummy_img, "chest_image": dummy_img, "wrist_image": dummy_img}, dummy_state)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--device", default="cuda:0", help="PyTorch/CUDA device.")
    ...
    p.add_argument("--trt-engine", default=None,
                   help="Load a Torch-TensorRT .ep engine for sample_actions.")
    p.add_argument("--trt-build-engine", default=None,
                   help="Compile sample_actions to TensorRT at startup and save it to this .ep path.")
    p.add_argument("--trt-num-steps", type=int, default=10,
                   help="Flow sampling steps baked into the TensorRT engine.")
    p.add_argument("--trt-fp32", action="store_true",
                   help="Build TensorRT in FP32 instead of FP16.")
...
def main() -> None:
    args = parse_args()
    if args.trt_engine and args.trt_build_engine:
        raise ValueError("--trt-engine and --trt-build-engine are mutually exclusive.")
    ...
    inferencer = Pi05Inference(
        ...
        trt_build_engine=args.trt_build_engine,
        trt_num_steps=args.trt_num_steps,
    ...
    if args.trt_engine:
        print(f"TensorRT       : load {args.trt_engine}")
    elif args.trt_build_engine:
        print(f"TensorRT       : build {args.trt_build_engine}")
