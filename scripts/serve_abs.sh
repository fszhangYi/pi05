...
# CONFIG_PATH="${1:-$ROOT_DIR/configs/pi05_tonglu0602_mlu_v2.yaml}"
# CONFIG_PATH="${1:-$ROOT_DIR/configs/pi05_tonglu0602_mlu_example_terminal_abs_pose_filter.yaml}"
CONFIG_PATH="${1:-$ROOT_DIR/configs/pi05_tonglu0630_mlu_example_terminal_abs_pose.yaml}"
CHECKPOINT_STEP="${2:-}"
PORT="${PORT:-5000}"
TEMPORAL_AGG="${TEMPORAL_AGG:-0}"
NUM_STEPS="${NUM_STEPS:-}"
TASK_PROMPT="${TASK_PROMPT:-}"
IMAGE_PROTOCOL="${IMAGE_PROTOCOL:-legacy}"
USE_VENV="${USE_VENV:-0}"
...
CMD=("$PYTHON_CMD" -m pi05_jax_sft.serve_abs_pose --config "$CONFIG_PATH" --port "$PORT")
